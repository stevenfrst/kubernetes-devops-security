pipeline {
  ...
  environment {
    ...
    imageName = "stevenfrst/numeric-app:${GIT_COMMIT}"
    applicationURL = "http://1693d5ddf82c.mylabserver.com/"
    ...
    stage('Vulnerability Scan - Docker') {
      steps {
        parallel(
          ...
          "OPA Conftest": {
            sh 'docker run --rm -v $(pwd):/project openpolicyagent/conftest test --policy opa-docker-sec.rego Dockerfile'
          }
        )
      }
    }
    ...
    stage('Docker Build and Push') {
      steps {
        withDockerRegistry([credentialsId: "docker-creds", url: ""]) {
          sh 'printenv'
          sh 'sudo docker build -t stevenfrst/numeric-app:""$GIT_COMMIT"" .'
